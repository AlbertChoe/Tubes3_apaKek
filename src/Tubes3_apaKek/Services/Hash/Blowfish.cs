...
public class Blowfish
{
    private string[][] S = new string[][] { 
      new string[]{ "d1310ba6", "98dfb5ac", "2ffd72db", "d01adfb7", "b8e1afed",
              ...
              "6e85076a" },
            new string[]{ "4b7a70e9", "b5b32944", "db75092e", "c4192623", "ad6ea6b0",
              ...
              "db83adf7" },
            new string[]{ "e93d5a68", "948140f7", "f64c261c", "94692934", "411520f7",
              ...
              "406000e0" },
            new string[]{ "3a39ce37", "d3faf5cf", "abc27737", "5ac52d1b", "5cb0679e",
    ...
    private long modVal = 1L << 32;

    private Vignere vignere;

    public Blowfish(string key)
    {
      vignere = new Vignere(key);
      KeyGenerate(key);
    }
    ...
    private string HexToBin(string hex)
    {
      ...
      foreach (char c in hex)
      {
        ...
        string binary4B = Convert.ToString(num, 2).PadLeft(4, '0');
        binary += binary4B;
      }
      return binary;
    }
    ...
    private string BinToHex(string binary)
    {
      ...
      string hex = Convert.ToString(num, 16).PadLeft(binary.Length / 4, '0');
      return hex;
    }
    ...
    {
      long n1 = Convert.ToInt64(a, 16);
      long n2 = Convert.ToInt64(b, 16);
      long sum = (n1 + n2) % modVal;
    ...
    private string F(string plainText)
    {
      ...
      for (int i = 0; i < 8; i += 2)
      {
        long col = Convert.ToInt64(HexToBin(plainText.Substring(i, 2)), 2);
        a[i / 2] = S[i / 2][col];
      }
      ans = AddBin(a[0], a[1]);
      ans = Xor(ans, a[2]);
    ...
    private string Round(int time, string plainText)
    {
      ...
      string right = plainText.Substring(8, 8);
      left = Xor(left, P[time]);
      string fOut = F(left); 
      right = Xor(fOut, right);

      // Console.WriteLine($"round {time}: {right}{left}");
    ...
    public string Encrypt(string plainText)
    {
      ...
      string hexText = BitConverter.ToString(plainBytes).Replace("-", "").ToLower();

      int length = hexText.Length;
      int maxLength = (length / 16) * 16;
      
      string blowfishText = hexText.Substring(0, maxLength);
      string vignereText = hexText.Substring(maxLength);

      StringBuilder encryptedText = new StringBuilder();
      for (int i = 0; i < blowfishText.Length; i += 16)
      {
        string block = blowfishText.Substring(i, 16);
        for (int round = 0; round < 16; round++)
          block = Round(round, block);

        string right = block.Substring(0, 8);
        string left = block.Substring(8, 8);
      ...
      string vignereTextE = vignere.Encrypt(vignereText);

      encryptedText.Append(vignereTextE);
    ...
    public string Decrypt(string encryptedText)
    {
      ...
      string blowfishText = encryptedText.Substring(0, maxLength);
      string vignereText = encryptedText.Substring(maxLength);
      ...
      for (int i = 0; i < blowfishText.Length; i += 16)
      {
        ...
        decryptedHex.Append(left + right);
      }
      
      string vignereTextD = vignere.Decrypt(vignereText);

      decryptedHex.Append(vignereTextD);

      byte[] bytes = new byte[decryptedHex.Length / 2];
      for (int i = 0; i < decryptedHex.Length; i += 2)
        bytes[i / 2] = Convert.ToByte(decryptedHex.ToString().Substring(i, 2), 16);
    ...
    {
      string tes = "HAloa ./ ahah apakah itu";
      string key = "aabb09182736ccdd";
      Blowfish b = new Blowfish(key);
      string e = b.Encrypt(tes);
      Console.WriteLine("en");

      Console.WriteLine(e);

      string d = b.Decrypt(e);
