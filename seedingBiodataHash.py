...
class Blowfish:
    ...
    def bin_to_hex(self, bin_val):
        return hex(int(bin_val, 2))[2:].zfill(len(bin_val) // 4)

    def generate_key_cipher(self, hex_string, key):
        key = list(key)
        if len(hex_string) == len(key):
            return ''.join(key)
        else:
            for i in range(len(hex_string) - len(key)):
    ...
    def key_generate(self, key):
        ...
        for i in range(len(self.P)):
            p_xor = self.xor(self.P[i], key[j:j+8])
            # print(f"subkey {i + 1}: {p_xor}")
            self.P[i] = p_xor
            j = (j + 8) % len(key)
    def round(self, time, plain_text):
        left = plain_text[:8]
        right = plain_text[8:]
        left = self.xor(left, self.P[time])
        f_out = self.F(left)
        right = self.xor(f_out, right)
        # print(f"round {time}: {right}{left}")
        return right + left
    def xor(self, a, b):
        a_bin = self.hex_to_bin(a)
        b_bin = self.hex_to_bin(b)
        result = int(a_bin, 2) ^ int(b_bin, 2)
        return self.bin_to_hex(bin(result)[2:].zfill(len(a_bin)))
    ...
    def F(self, plain_text):
        a = []
        for i in range(0, 8, 2):
            col = int(self.hex_to_bin(plain_text[i:i+2]), 2)
            a.append(self.S[i // 2][col])
        ans = self.add_bin(a[0], a[1])
        ans = self.xor(ans, a[2])
        ans = self.add_bin(ans, a[3])
        return ans

    def cipher_text(self, hex_string, key):
        cipher_text = []
        for i in range(len(hex_string)):
            x = (int(hex_string[i], 16) + int(key[i], 16)) % 16
            cipher_text.append(format(x, 'x'))
        return ''.join(cipher_text)

    def original_text(self, cipher_text, key):
        orig_text = []
        for i in range(len(cipher_text)):
            x = (int(cipher_text[i], 16) - int(key[i], 16) + 16) % 16
            orig_text.append(format(x, 'x'))
        return ''.join(orig_text)
    
    def encrypt(self, plain_text):
        plain_bytes = plain_text.encode('utf-8')
        hex_text = ''.join(format(byte, '02x') for byte in plain_bytes)

        length = len(hex_text)
        max_length = (length // 16) * 16
        
        vignere_text = hex_text[max_length:]
        blowfish_text = hex_text[:max_length]

        encrypted_text = ''
        for i in range(0, len(blowfish_text), 16):
            block = blowfish_text[i:i + 16]
            for round in range(16):
                block = self.round(round, block)
        ...
        vignere_text_e = self.cipher_text(vignere_text, self.generate_key_cipher(vignere_text, self.key))

        return encrypted_text + vignere_text_e
    ...
    def decrypt(self, encrypted_text):
        ...
        vignere_text = encrypted_text[max_length:]
        blow_text = encrypted_text[:max_length]
        
        decrypted_text = ''
        for i in range(0, len(blow_text), 16):
            block = blow_text[i:i + 16]
            for round in range(17, 1, -1):
                block = self.round(round, block)
        ...
        vignere_text_d = self.original_text(vignere_text, self.generate_key_cipher(vignere_text, self.key))
        
        decrypted_text += vignere_text_d

        bytes_ = bytes(int(decrypted_text[i:i + 2], 16) for i in range(0, len(decrypted_text), 2))
...
def process_names():
    ...
    for (original_name,) in names:
        NIK = hash.encrypt(str(fake.unique.random_number(digits=16, fix_len=True)))
        corrupted_name = hash.encrypt(corrupt_name(hash.decrypt(original_name)))
        tempat_lahir = hash.encrypt(fake.city())
        tanggal_lahir = fake.date_of_birth(minimum_age=18, maximum_age=65)
        jenis_kelamin = random.choice(['Laki-Laki', 'Perempuan'])
        golongan_darah = hash.encrypt(random.choice(
            ['A', 'B', 'AB', 'O']) + random.choice(['+', '-']))
        alamat = hash.encrypt(fake.address())
        agama = hash.encrypt(random.choice(
            ['Islam', 'Kristen', 'Hindu', 'Buddha', 'Konghucu']))
        status_perkawinan = random.choice(
            ['Belum Menikah', 'Menikah', 'Cerai'])
        pekerjaan = hash.encrypt(fake.job())
        kewarganegaraan = hash.encrypt('Indonesia')
...
if __name__ == "__main__":
    ...
    # plain_text = "Gang Dr. Djunjunan No. 60\nBekasi, Kalimantan Selatan 02089"
    # hex_plain_text = text_to_hex(plain_text)
    # key = "BENSACHBERT"
    # hex_key = text_to_hex(key)
    # 
    # gen_key = generate_key(hex_plain_text, hex_key)
    # 
    # encrypted_text = cipher_text(hex_plain_text, gen_key)
    # print("Encrypted:", encrypted_text)
    # 
    # # Decrypt
    # decrypted_text = original_text(encrypted_text, gen_key)
    # # Convert hex back to plain text
    # plain_text_output = hex_to_text(decrypted_text)
    # print("Decrypted:", plain_text_output)
    # hash = Blowfish()
    # hash.key_generate("aabb09182736ccdd")
    # 
    # encrypted_text = hash.encrypt(plain_text)
    # print("Encrypted:", encrypted_text)
    # 
    # decrypted_text = hash.decrypt(encrypted_text)
